...
export class LeagueSearchPage implements OnInit {
  ...
  public data: any[] = [];
  public searchVal: string = 'Italian Serie A';
  ...
  protected async getLeague(val: string): Promise<void> {
    ...
    if (this.searchVal.toLowerCase() !== 'italian serie a') {
    ...
    try {
      const res = await fetch(
        `https://www.thesportsdb.com/api/v1/json/3/search_all_teams.php?l=${val}`
      );
